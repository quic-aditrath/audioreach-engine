...
static ar_result_t wcntr_topo_capi_get_stack_size(void *           amdb_handle,
                                                uint32_t         log_id,
                                                uint32_t *       stack_size,
                                                capi_proplist_t *init_proplist_ptr)
{
   ...
   *stack_size = stack_size_struct.size_in_bytes;
   ...
   CATCH(err_code, WCNTR_TOPO_MSG_PREFIX, log_id)
   {
...
static ar_result_t wcntr_topo_capi_is_inplace_n_requires_data_buf(void *           amdb_handle,
   ...
   *requires_data_buf = FALSE;
   ...
   if (CAPI_FAILED(err_code))
   {
      WCNTR_TOPO_MSG(log_id,
                     DBG_ERROR_PRIO,
                     "Module 0x%lX: Get properties error. Assuming capi is not inplace, and does not require data "
                     "buffering.",
                     module_instance_id);
   ...
   else
   {
      ...
      if (rdf.requires_data_buffering)
      {
         WCNTR_TOPO_MSG(log_id,
                        DBG_ERROR_PRIO,
                        "Module 0x%lX: requires-data-buffering but is not supported",
                        module_instance_id);
         err_code = AR_EFAILED;
      }
   }
                                                       ...
                                                       capi_proplist_t *init_proplist_ptr)
{
   ...
   wcntr_topo_module_t *            module_ptr              = (wcntr_topo_module_t *)module_ctx_ptr;
   wcntr_topo_t *                   topo_ptr                = (wcntr_topo_t *)topo_ctx_ptr;
   ...
   uint32_t log_id             = module_ptr->topo_ptr->gu.log_id;
       ...
       WCNTR_TOPO_MSG(log_id,
                     DBG_HIGH_PRIO,
                     "querying Module 0x%lX for fwk extensions ",
                     module_instance_id);
   ...
   if ((num_fmwk_extns.num_extensions != 0) && (CAPI_SUCCEEDED(err_code)))
   {
      ...
      for (i = 0; i < num_fmwk_extns.num_extensions; i++)
      {
         ...
         switch (needed_fmwk_xtn_ids_arr[i].id)
         {
            case FWK_EXTN_STM:
            {
               ...
               {
                  WCNTR_TOPO_MSG(topo_ptr->gu.log_id, DBG_ERROR_PRIO, "More than one module has signal trigger");
         ...
         if (!extn_supported)
         {
            WCNTR_TOPO_MSG(log_id,
                     DBG_ERROR_PRIO,
                     "Fmwk extension 0x%lx requested by Module 0x%lX: not supported. Module may not work correctly",
                     needed_fmwk_xtn_ids_arr[i].id,module_instance_id);
         ...
         else
         {
            WCNTR_TOPO_MSG(log_id,
                     DBG_HIGH_PRIO,
                     "Module 0x%lX needs fmwk extension 0x%lX which is supported",
                     module_instance_id,
      ...
      if (any_extn_not_supported)
      {
         WCNTR_TOPO_MSG(log_id,
                  DBG_ERROR_PRIO,
                  "Module 0x%lX Failing since the fmwk doesnot support some extensions (Warning) ",
                  module_instance_id);
         THROW(result, AR_EFAILED);
      }
   }
...
static ar_result_t wcntr_topo_capi_query_intf_extn_support(void *             amdb_handle,
...
// clang-format off
   ...
   typedef struct wcntr_topo_module_intf_extn_list_t
   {
      capi_interface_extns_list_t intf_extn_num;
      capi_interface_extn_desc_t  intf_extn_desc[LEN_OF_INTF_EXTNS_ARRAY];
   } wcntr_topo_module_intf_extn_list_t;

   wcntr_topo_module_intf_extn_list_t intf_ext_list = {
   ...
   capi_prop_t     prop[1]; // query only one property
    ...
    WCNTR_TOPO_MSG(log_id,
                           DBG_HIGH_PRIO,
                           "capi_query_intf_extn_support querying module 0x%lX",
                           module_instance_id);

   prop[0].id                      = CAPI_INTERFACE_EXTENSIONS;
   prop[0].payload.actual_data_len = sizeof(wcntr_topo_module_intf_extn_list_t);
   prop[0].payload.max_data_len    = sizeof(wcntr_topo_module_intf_extn_list_t);
         ...
         if (intf_ext_list.intf_extn_desc[j].is_supported)
         {
            ...
            switch (intf_ext_list.intf_extn_desc[j].id)
            {
               ...
               default:
               {
                  ...
                  WCNTR_TOPO_MSG(log_id,
                           DBG_ERROR_PRIO,
                           "Warning: Module 0x%lx supports interface extension 0x%lX but fwk "
                           "cannot support it",
                           module_instance_id, intf_ext_list.intf_extn_desc[j].id);
                  break;
               }
            }
         }
                                           ...
                                           wcntr_topo_graph_init_t *graph_init_ptr)
{
   ...
   wcntr_topo_set_bits(&temp_log_id,
                     module_ptr->serial_num,
                     WCNTR_LOG_ID_LOG_MODULE_INSTANCES_MASK,
                     WCNTR_LOG_ID_LOG_MODULE_INSTANCES_SHIFT);

#ifdef HEAP_PROFILING
   mem_heap_id = temp_log_id | mem_heap_id;
   WCNTR_TOPO_MSG(log_id, DBG_LOW_PRIO, "Module 0x%lX: mem_heap_id 0x%lX", module_instance_id, mem_heap_id);
#endif
   ...
   capi_logging_info_t logging_info = {.log_id = temp_log_id, .log_id_mask = WCNTR_LOG_ID_LOG_DISCONTINUITY_MASK };

   uint32_t i = 0;

   init_props[i].id                      = CAPI_EVENT_CALLBACK_INFO;
   init_props[i].payload.actual_data_len = sizeof(cb_obj);
   init_props[i].payload.max_data_len    = sizeof(cb_obj);
   init_props[i].payload.data_ptr        = (int8_t *)(&cb_obj);
   init_props[i].port_info.is_valid      = FALSE;
   i++;

   init_props[i].id                      = CAPI_HEAP_ID;
   init_props[i].payload.actual_data_len = sizeof(heap_id);
   init_props[i].payload.max_data_len    = sizeof(heap_id);
   init_props[i].payload.data_ptr        = (int8_t *)(&heap_id);
   init_props[i].port_info.is_valid      = FALSE;
   i++;

   init_props[i].id                      = CAPI_PORT_NUM_INFO;
   init_props[i].payload.actual_data_len = sizeof(num_max_ports_info);
   init_props[i].payload.max_data_len    = sizeof(num_max_ports_info);
   init_props[i].payload.data_ptr        = (int8_t *)(&num_max_ports_info);
   init_props[i].port_info.is_valid      = FALSE;
   i++;

   init_props[i].id                      = CAPI_MODULE_INSTANCE_ID;
   init_props[i].payload.actual_data_len = sizeof(miid);
   init_props[i].payload.max_data_len    = sizeof(miid);
   init_props[i].payload.data_ptr        = (int8_t *)(&miid);
   init_props[i].port_info.is_valid      = FALSE;
   i++;

   init_props[i].id                      = CAPI_LOGGING_INFO;
   init_props[i].payload.actual_data_len = sizeof(logging_info);
   init_props[i].payload.max_data_len    = sizeof(logging_info);
   init_props[i].payload.data_ptr        = (int8_t *)(&logging_info);
   init_props[i].port_info.is_valid      = FALSE;
   i++;

   VERIFY(err_code, (i <= NUM_INIT_PARAMS));

   init_proplist.props_num = i;
   init_proplist.prop_ptr  = init_props;
   ...
   TRY(result, wcntr_topo_capi_get_stack_size(amdb_handle, topo_ptr->gu.log_id, &stack_size, &init_proplist));

   graph_init_ptr->max_stack_size = MAX(graph_init_ptr->max_stack_size, stack_size);

   wcntr_topo_capi_is_inplace_n_requires_data_buf(amdb_handle,
   ...
   TRY(result,
       wcntr_topo_capi_get_required_fmwk_extensions((void *)topo_ptr, (void *)module_ptr, amdb_handle, &init_proplist));

   // Check if the module supports any of the available interface extensions
   wcntr_topo_capi_query_intf_extn_support(amdb_handle, module_ptr, &init_proplist);
                                          ...
                                          uint32_t *thresh)
{
   ...
   *thresh = 1;
   ...
   WCNTR_TOPO_MSG(log_id,
            DBG_HIGH_PRIO,
            "Module 0x%lX: port-threshold for is_input %u, port_index %u, is %lu",
            module_instance_id,
            is_input,
            port_index,
...
static ar_result_t wcntr_topo_set_media_fmt_v1(wcntr_topo_t *       topo_ptr,
                                             wcntr_topo_module_t *module_ptr,
                                             ...
                                             uint16_t           port_index)
{
   ...
   wcntr_topo_capi_media_fmt_t  capi_media_fmt;
   wcntr_topo_capi_media_fmt_t *capi_media_fmt_ptr = &capi_media_fmt;

   VERIFY(err_code, NULL != capi_ptr);

   capi_media_fmt_ptr->main.format_header.data_format =
      wcntr_topo_convert_spf_data_format_to_capi_data_format(media_fmt_ptr->data_format);
   ...
   if (SPF_IS_PACKETIZED_OR_PCM(media_fmt_ptr->data_format))
   {
      ...
      capi_media_fmt_ptr->std.data_interleaving =
         wcntr_topo_convert_wcntr_topo_interleaving_to_capi_interleaving(media_fmt_ptr->pcm.interleaving);
   ...
   {
      AR_MSG(DBG_ERROR_PRIO, "wcntr_topo_capi_set_media_fmt: unsupported format ");
   }
   ...
   if (CAPI_FAILED(err_code))
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "setting media fmt 0x%lx failed", prop_id);
   }
...
static ar_result_t wcntr_topo_set_media_fmt_v2(wcntr_topo_t *       topo_ptr,
                                             wcntr_topo_module_t *module_ptr,
                                             wcntr_topo_media_fmt_t * media_fmt_ptr,
                                             capi_property_id_t prop_id,
                                             uint16_t           port_index)
{
   ...
   wcntr_topo_capi_media_fmt_v2_t  capi_media_fmt;
   wcntr_topo_capi_media_fmt_v2_t *capi_media_fmt_ptr = &capi_media_fmt;
   ...
   if (CAPI_FAILED(err_code))
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "Warning: setting media fmt 0x%lx failed", prop_id);
   }
                                        ...
                                        uint16_t           port_index)
{
   ...
   {
      AR_MSG(DBG_ERROR_PRIO, "wcntr_topo_capi_set_media_fmt: Received NULL pointer(s)");
      return AR_EBADPARAM;
   }
   capi_err_t result = AR_EOK;

   AR_MSG(DBG_HIGH_PRIO,
          "topo_capi_set_media_fmt: START module_ptr 0x%lX ,is_input_mf %u, port_index %u ",
          module_ptr->gu.module_instance_id,
          is_input_mf,
          port_index);
   ...
   if (is_input_mf)
   {
      ...
      if (AR_EOK !=
          (result =
              wcntr_topo_set_media_fmt_v2(topo_ptr, module_ptr, media_fmt_ptr, CAPI_INPUT_MEDIA_FORMAT_V2, port_index)))
      {
         WCNTR_TOPO_MSG(topo_ptr->gu.log_id, DBG_ERROR_PRIO, "Module failed V2 set media format, trying V1");
         if (AR_EOK !=
             (result =
                 wcntr_topo_set_media_fmt_v1(topo_ptr, module_ptr, media_fmt_ptr, CAPI_INPUT_MEDIA_FORMAT, port_index)))
         {
            WCNTR_TOPO_MSG(topo_ptr->gu.log_id, DBG_ERROR_PRIO, "Set input media format failed, result = %lu", result);
            return AR_EFAILED;
         }
      }
   }
   else // output set prop
   {
      ...
      if (AR_EOK !=
          (result =
              wcntr_topo_set_media_fmt_v2(topo_ptr, module_ptr, media_fmt_ptr, CAPI_OUTPUT_MEDIA_FORMAT_V2, port_index)))
      {
         WCNTR_TOPO_MSG(topo_ptr->gu.log_id, DBG_ERROR_PRIO, "Module failed V2 set media format, trying V1");
         if (AR_EOK !=
             (result =
                 wcntr_topo_set_media_fmt_v1(topo_ptr, module_ptr, media_fmt_ptr, CAPI_OUTPUT_MEDIA_FORMAT, port_index)))
         {
            WCNTR_TOPO_MSG(topo_ptr->gu.log_id, DBG_ERROR_PRIO, "Set output media format failed, result = %lu", result);
            return AR_EFAILED;
         }
      }
   }

   AR_MSG(DBG_HIGH_PRIO, "topo_capi_set_media_fmt END for module_ptr 0x%lX ", module_ptr->gu.module_instance_id);
   return result;
}

static data_format_t wcntr_topo_get_out_data_format_by_module_type(uint16_t module_type)
{
...
static void wcntr_topo_assign_default_out_medifa_fmt(wcntr_topo_module_t *           module_ptr,
                                                   wcntr_topo_capi_media_fmt_v2_t *media_fmt_ptr)
{
   switch (module_ptr->gu.module_type)
   {
      case AMDB_MODULE_TYPE_GENERIC:
      {
         if (module_ptr->gu.flags.is_siso)
         {
            ...
            wcntr_topo_media_fmt_t *topo_media_fmt_ptr = &in_port_ptr->common.media_fmt;

            media_fmt_ptr->main.format_header.data_format =
               wcntr_topo_convert_spf_data_format_to_capi_data_format(topo_media_fmt_ptr->data_format);
            ...
            media_fmt_ptr->std.fmt.data_interleaving =
               wcntr_topo_convert_wcntr_topo_interleaving_to_capi_interleaving(topo_media_fmt_ptr->pcm.interleaving);
            ...
            WCNTR_TOPO_MSG(module_ptr->topo_ptr->gu.log_id, DBG_LOW_PRIO, "Assuming output media format same as input");
         }
         break;
      }
      default:
      {
         media_fmt_ptr->main.format_header.data_format =
            wcntr_topo_get_out_data_format_by_module_type(module_ptr->gu.module_type);
                                            ...
                                            wcntr_topo_output_port_t *out_port_ptr)
{
   ...
   WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
            DBG_LOW_PRIO,
            "capi_get_out_media_fmt for module 0x%lX START",
            module_ptr->gu.module_instance_id);
   ...
   union
   {
      wcntr_topo_capi_media_fmt_v2_t v2;
      wcntr_topo_capi_media_fmt_t    v1;
   } capi_media_fmt;
   ...
   out_media_fmt_size_struct.size_in_bytes = sizeof(wcntr_topo_capi_media_fmt_v2_t) - sizeof(capi_data_format_header_t);
   ...
   if (CAPI_FAILED(err_code))
   {
      ...
      total_size_v2 = sizeof(wcntr_topo_capi_media_fmt_v2_t);
      total_size_v1 = sizeof(wcntr_topo_capi_media_fmt_t);
   }
   else
   {
   ...
   // size
   if (total_size_v2 <= sizeof(wcntr_topo_capi_media_fmt_v2_t))
   {
// query for v2
      ...
      prop[0].payload.actual_data_len = 0;
      prop[0].payload.max_data_len    = sizeof(wcntr_topo_capi_media_fmt_v2_t);
   ...
   else
   {
      WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
               ...
               total_size_v2,
               sizeof(wcntr_topo_capi_media_fmt_v2_t));
   }
   // if it fails, query for v1
   if (CAPI_FAILED(err_code))
   {
      if (total_size_v1 <= sizeof(wcntr_topo_capi_media_fmt_t))
      {
         is_std_fmt_v2 = FALSE;
#ifdef VERBOSE_DEBUGGING
         WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
         ...
         prop[0].payload.actual_data_len = 0;
         prop[0].payload.max_data_len    = sizeof(wcntr_topo_capi_media_fmt_t);
         ...
         if (CAPI_FAILED(err_code))
         {
            wcntr_topo_assign_default_out_medifa_fmt(module_ptr, &capi_media_fmt.v2);
            WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
                     DBG_ERROR_PRIO,
      ...
      else
      {
         WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
                  ...
                  out_port_ptr->gu.cmn.index,
                  total_size_v1,
                  sizeof(wcntr_topo_capi_media_fmt_t));
         return AR_EFAILED;
      }
   }
   ...
   result = capi_err_to_ar_result(
      wcntr_topo_handle_output_media_format_event(topo_ptr, module_ptr, &event_info, is_std_fmt_v2, FALSE));

   WCNTR_TOPO_MSG(topo_ptr->gu.log_id,
            DBG_LOW_PRIO,
            "capi_get_out_media_fmt module 0x%lX END with result %u",
            module_ptr->gu.module_instance_id,result);
                                            ...
                                            uint16_t port_index)
{
   ...
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "capi algorithmic reset failed");
      return result;
   }
   result = CAPI_EOK;
                                    ...
                                    uint32_t *size_ptr)
{
   ...
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "get param for (param id 0x%lx) result %d", param_id, result);
   }
   else
   {
      // WCNTR_TOPO_MSG(log_id, DBG_LOW_PRIO, "get param for (param id 0x%lx) success", param_id);
   }
                                    ...
                                    uint32_t size)
{
   ...
   if (CAPI_FAILED(result))
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "set param for (param id 0x%lx, size %lu) result %d", param_id, size, result);
   }
   else
   {
      // WCNTR_TOPO_MSG(log_id, DBG_LOW_PRIO, "set param for (param id 0x%lx, size %lu) success", param_id, size);
   }
...
static ar_result_t wcntr_topo_capi_register_event_v1(uint32_t           log_id,
                                                   capi_t *           capi_ptr,
                                                   wcntr_topo_module_t *module_ptr,
                                                   ...
                                                   bool_t             is_register)
{
   ...
   if (CAPI_FAILED(result))
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "Set property for register events failed, result: %d", result);
...
static ar_result_t wcntr_topo_capi_register_event_v2(uint32_t          log_id,
                                                   capi_t *          capi_ptr,
                                                   wcntr_topo_reg_event_t *reg_event_payload_ptr,
                                                   bool_t            is_register)
{
    ...
    * bits 0-31:src port,   bits 32-39:src domain id,   bits 40-47:dest domain id,   bits 48-63:0 */
   ...
   if (CAPI_FAILED(result))
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "Set property for register events v2 failed, result: %d", result);
   }
                                                        ...
                                                        bool_t *           store_client_info_ptr)
{
   ...
   *store_client_info_ptr = FALSE;
   ...
   if (AR_EOK != (result = wcntr_topo_capi_register_event_v2(log_id, capi_ptr, reg_event_payload_ptr, is_register)))
   {
      if (0 == reg_event_payload_ptr->event_cfg.actual_data_len)
      {
         WCNTR_TOPO_MSG(log_id, DBG_HIGH_PRIO, "Trying V1 register event");
         if (AR_EOK !=
             (result =
                 wcntr_topo_capi_register_event_v1(log_id, capi_ptr, module_ptr, reg_event_payload_ptr, is_register)))
         {
            WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "Register event v1 failed, result = %lu", result);
            return result;
         }
         else
         {
      ...
      {
         WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "Module failed V2 register event and not trying V1");
      }
   }
...
void wcntr_topo_get_dl_info(uint32_t log_id, void *amdb_handle, bool_t *is_dl, uint32_t **start_addr, uint32_t *so_size)
{
                                               ...
                                               spf_cfg_data_type_t cfg_type)
{
   ...
   {
      WCNTR_TOPO_MSG(log_id, DBG_ERROR_PRIO, "setting PARAM_PERSISTENCE_INFO failed");
      return capi_err_to_ar_result(err_code);
   }
   return AR_EOK;
}
