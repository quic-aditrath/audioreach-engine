...
// and computes the additional bytes required to accomodate variable buffer size requriement of the DM module
                                                                ...
                                                                gen_topo_input_port_t *in_port_ptr)
{
   ...
   uint32_t additional_bytes_req_for_dm = 0;
   if (gen_topo_is_in_port_in_dm_variable_nblc(topo_ptr, in_port_ptr))
   {
      additional_bytes_req_for_dm =
         topo_us_to_bytes(GEN_TOPO_DM_BUFFER_MAX_EXTRA_LEN_US, in_port_ptr->common.media_fmt_ptr);

      TOPO_MSG(topo_ptr->gu.log_id,
               DBG_HIGH_PRIO,
               "Module 0x%lX: in port id 0x%lx, additional_bytes=%lu to handle DM modules variable "
               "input/output",
   ...
   return additional_bytes_req_for_dm;
}
                                                                 ...
                                                                 gen_topo_output_port_t *out_port_ptr)
{
   ...
   {
      additional_bytes_req_for_dm =
         topo_us_to_bytes(GEN_TOPO_DM_BUFFER_MAX_EXTRA_LEN_US, out_port_ptr->common.media_fmt_ptr);

      TOPO_MSG(topo_ptr->gu.log_id,
               DBG_HIGH_PRIO,
               "Module 0x%lX: out port id 0x%lx, additional_bytes=%lu to handle DM modules variable "
               "input/output",
