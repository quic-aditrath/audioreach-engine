                                        ...
                                        capi_media_fmt_v2_t *  media_fmt_ptr)
{
   ...
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_spr: received invalid params to create int_buf node");
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "Failed to allocate stream data struct for int buf node");
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "Failed to allocate capi buf data struct for hold node");
   ...
   SPR_MSG(me_ptr->miid, DBG_LOW_PRIO, "alloc int_buf_obj_ptr 0x%X", int_buf_obj_ptr);
#endif
   ...
   for (uint32_t num_chan = 0; num_chan < int_buf_obj_ptr->bufs_num; num_chan++)
   {
      ...
      else
      {
         ...
         {
            SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "Failed to allocate capi data buffer");
                                         ...
                                         bool_t               drop_data)
{
   ...
   if (check_if_head_node)
   {
      ...
      {
#ifdef SPR_INT_BUF_DEBUG
         SPR_MSG(me_ptr->miid, DBG_LOW_PRIO, "Incoming node 0x%p not head of the provided list", buf_node_ptr);
#endif
         return CAPI_EOK;
      }
...
#ifdef SPR_INT_BUF_DEBUG
      SPR_MSG(me_ptr->miid, DBG_LOW_PRIO, "Incoming node 0x%p head of the provided list", buf_node_ptr);
#endif
   }

   if (strm_ptr->metadata_list_ptr)
   {
      SPR_MSG(me_ptr->miid, DBG_HIGH_PRIO, "warning: int buffer node being destroyed with metadata");
   }

#ifdef SPR_INT_BUF_DEBUG
   SPR_MSG(me_ptr->miid, DBG_LOW_PRIO, "destroy int_buf_obj_ptr 0x%X", strm_ptr);
...
#ifdef SPR_INT_BUF_DEBUG
   SPR_MSG(me_ptr->miid, DBG_LOW_PRIO, "int_buf cur fill size %d", int_buf_ptr->cur_fill_size_us);
#endif
