...
capi_err_t capi_spr_avsync_reset_session_clock_for_gapless(avsync_t *avsync_ptr)
{
   ...
   SPR_MSG(avsync_ptr->miid, DBG_HIGH_PRIO, "avsync: session time reset received from gapless");
...
//#ifdef AVSYNC_DEBUG
   SPR_MSG(avsync_ptr->miid, DBG_HIGH_PRIO,
...
capi_err_t capi_spr_avsync_update_session_clock(avsync_t *avsync_ptr, int64_t adj_bytes, uint32_t sr, uint32_t bps)
{
   ...
   {
#ifdef AVSYNC_DEBUG
      AR_MSG(DBG_ERROR_PRIO, "capi_spr: avsync: Received invalid params for update session clock");
#endif
      return CAPI_EOK;
   }
...
capi_err_t capi_spr_avsync_update_absolute_time(avsync_t *avsync_ptr, int64_t adj_us)
{
   ...
   {
#ifdef AVSYNC_DEBUG
      AR_MSG(DBG_ERROR_PRIO, "capi_spr: avsync: Received invalid params for update absolute time ");
#endif
      return CAPI_EBADPARAM;
   }
...
#ifdef AVSYNC_DEBUG
      SPR_MSG(me_ptr->miid, DBG_MED_PRIO, "avsync: sess_clk_debug num_bytes_filled = %d", num_bytes_filled);
#endif
...
capi_err_t capi_spr_avsync_setup_first_input(capi_spr_t *me_ptr, capi_stream_data_t *input)
{
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "avsync, avsync functions called without setting up primary out port.");
   }
   else
   {
...
#ifdef AVSYNC_DEBUG
   SPR_MSG(me_ptr->miid, DBG_HIGH_PRIO, "avsync: Received first buffer");
#endif
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "avsync: Warning! Process is called but primary output port is not set!");
   }
   else
   {
...
#ifdef AVSYNC_DEBUG
   SPR_MSG(me_ptr->miid, DBG_HIGH_PRIO, "avsync: Ref Session Time(%ld)", input_timestamp_us);
#endif
   return;
}
...
static void spr_calculate_session_start_time(capi_spr_t *me_ptr, capi_stream_data_t *input)
{
   ...
   switch (avsync_ptr->client_config.render_mode)
   {
      ...
      case SPR_RENDER_MODE_IMMEDIATE:
      {
         ...
         if (SPR_RENDER_REFERENCE_WALL_CLOCK == avsync_ptr->client_config.render_reference)
         {
            ...
            SPR_MSG(me_ptr->miid, DBG_HIGH_PRIO, "avsync: Configured for immediate run with Wall Clock Reference");
         }
         else
         {
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "avsync: Warning! session_start_time_us is negative. Making it zero");
...
#ifdef AVSYNC_DEBUG
   SPR_MSG(me_ptr->miid, DBG_HIGH_PRIO, "avsync: Render decision %lu (render1,hold2,drop3)", *render_decision);
#endif
...
capi_err_t capi_spr_avsync_update_expected_session_time(avsync_t *avsync_ptr, int64_t adj_bytes, uint32_t sr, uint32_t bps)
{
   ...
   {
#ifdef AVSYNC_DEBUG
      SPR_MSG(avsync_ptr->miid, DBG_HIGH_PRIO, "avsync: First buffer not yet rendered. Skip update");
      return CAPI_EOK;
#endif
   }
...
capi_err_t capi_spr_add_input_to_hold_list(capi_spr_t *me_ptr, capi_stream_data_v2_t *input_ptr)
{
   ...
   {
      SPR_MSG(me_ptr->miid, DBG_ERROR_PRIO, "avsync: Failed to allocate hold buf node with result %d", result);
      return result;
   }
