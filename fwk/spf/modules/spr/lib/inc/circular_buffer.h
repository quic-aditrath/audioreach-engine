#ifndef __SPR_CIRC_BUF_UTILS_H
#define __SPR_CIRC_BUF_UTILS_H
...
#define circ_buf_alloc spr_circ_buf_alloc
#define circ_buf_register_client spr_circ_buf_register_client
#define circ_buf_read spr_circ_buf_read
#define circ_buf_write spr_circ_buf_write
#define circ_buf_read_adjust spr_circ_buf_read_adjust
#define circ_buf_read_client_resize spr_circ_buf_read_client_resize
#define circ_buf_query_unread_bytes spr_circ_buf_query_unread_bytes
#define circ_buf_memset spr_circ_buf_memset
#define circ_buf_deregister_client spr_circ_buf_deregister_client
#define circ_buf_free spr_circ_buf_free
/*
 ...
 * buffer parameters. The memory chunks are multiple memory allocations, each
 * smaller than the circular buffer size. The sum of the memory chunk sizes
 * equals the circular buffer size.  TODO: update the comments, handles are allocated by the stream apis.
 * return : circbuf_result
 */
 ...
 * bytes_read_ptr [out]         : Pointer to the number of bytes read
 * functionality : copies the samples_to_read number of samples to out_ptr
 * return : circbuf_result
 */
circbuf_result_t circ_buf_read(circ_buf_client_t *rd_handle,
                               int8_t *           out_ptr,
 ...
 * return : circbuf_result
 */
circbuf_result_t circ_buf_write(circ_buf_client_t *wr_handle,
                                int8_t *           inp_ptr,
 ...
 * return : circbuf_result
 */
circbuf_result_t circ_buf_read_adjust(circ_buf_client_t *rd_handle, uint32_t read_offset, uint32_t *un_read_bytes);
...
/*
 * Queries the number of unread bytes left in the circular buffer.
 * circ_buf_struct_ptr[in/out]  : pointer to a circular buffer structure
 * unread_bytes_ptr   [out]     : number of unread bytes left
 * functionality : Queries the number of unread bytes left in the circular buffer.
 * return : circbuf_result
 */
circbuf_result_t circ_buf_query_unread_bytes(circ_buf_client_t *client_ptr, uint32_t *unread_bytes_ptr);

#ifdef __cplusplus
}
#endif /*__cplusplus*/

#endif /*__SPR_CIRC_BUF_UTILS_H*/
