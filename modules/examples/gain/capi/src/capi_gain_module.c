 ...
 * -----------------------------------------------------------------------*/
static capi_err_t capi_gain_module_process(capi_t *_pif, capi_stream_data_t *input[], capi_stream_data_t *output[]);

static capi_err_t capi_gain_module_end(capi_t *_pif);

static capi_err_t capi_gain_module_set_param(capi_t *                _pif,
                                             uint32_t                param_id,
                                             const capi_port_info_t *port_info_ptr,
                                             capi_buf_t *            params_ptr);

static capi_err_t capi_gain_module_get_param(capi_t *                _pif,
                                             uint32_t                param_id,
                                             const capi_port_info_t *port_info_ptr,
                                             capi_buf_t *            params_ptr);

static capi_err_t capi_gain_module_set_properties(capi_t *_pif, capi_proplist_t *props_ptr);

static capi_err_t capi_gain_module_get_properties(capi_t *_pif, capi_proplist_t *props_ptr);

/* Function table for gain module*/
static capi_vtbl_t vtbl = { capi_gain_module_process,        capi_gain_module_end,
                            capi_gain_module_set_param,      capi_gain_module_get_param,
                            capi_gain_module_set_properties, capi_gain_module_get_properties };

/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_get_static_properties
 ...
 * -------------------------------------------------------------------------*/
capi_err_t capi_gain_module_get_static_properties(capi_proplist_t *init_set_properties,
                                                  capi_proplist_t *static_properties)
{
   ...
   {
      capi_result = capi_gain_module_process_get_properties((capi_gain_module_t *)NULL, static_properties);
      if (CAPI_FAILED(capi_result))
      {
         AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: get static properties failed!");
         return capi_result;
      }
      else
      {
         AR_MSG(DBG_HIGH_PRIO, "capi_gain_module: get static properties successful");
      }
   }
   else
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Get static properties received NULL bad pointer");
   }
...
/*------------------------------------------------------------------------
  Function name: capi_gain_module_init
  ...
  initialized here
 * -----------------------------------------------------------------------*/
capi_err_t capi_gain_module_init(capi_t *_pif, capi_proplist_t *init_set_properties)
{
   capi_err_t capi_result = CAPI_EOK;

   if (NULL == _pif || NULL == init_set_properties)
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Init received NULL pointers, failing");
      return CAPI_EBADPARAM;
   }

   /* Cast to gain module capi struct*/
   capi_gain_module_t *me_ptr = (capi_gain_module_t *)_pif;

   memset(me_ptr, 0, sizeof(capi_gain_module_t));

   /* Assign function table defined above*/
   me_ptr->vtbl.vtbl_ptr = &vtbl;

   /* Initialize the operating media format of the gain module*/
   capi_gain_module_module_init_media_fmt_v2(&me_ptr->operating_media_fmt);
   ...
   {
      capi_result = capi_gain_module_process_set_properties(me_ptr, init_set_properties);
      if (CAPI_FAILED(capi_result))
      {
         AR_MSG(DBG_ERROR_PRIO, "capi_gain_module:  Initialization Set Property Failed");
         return capi_result;
      }
   }

   AR_MSG(DBG_HIGH_PRIO, "capi_gain_module: Initialization completed !!");
   return capi_result;
}

/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_process
...
static capi_err_t capi_gain_module_process(capi_t *_pif, capi_stream_data_t *input[], capi_stream_data_t *output[])
{
   ...
   if ((!input[0]) && (!output[0]))
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Process: No input or output buffer provided");
      return CAPI_EFAILED;
   }
...
/*------------------------------------------------------------------------
 * Function name: capi_gain_module_end
...
static capi_err_t capi_gain_module_end(capi_t *_pif)
{
   ...
   if (NULL == _pif)
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: End received bad pointer, 0x%p", _pif);
   ...
   AR_MSG(DBG_HIGH_PRIO, "capi_gain_module: End done");
   return capi_result;
}

/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_set_param
                                             ...
                                             capi_buf_t *            params_ptr)
{
   ...
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: set param received NULL pointers");
      return CAPI_EBADPARAM;
   }
   ...
   switch (param_id)
   {
      ...
      case PARAM_ID_MODULE_ENABLE:
      {
         if (params_ptr->actual_data_len >= sizeof(param_id_module_enable_t))
         {
            ...
            capi_result |= capi_gain_module_raise_process_event(me_ptr, me_ptr->gain_config.enable);

            AR_MSG(DBG_HIGH_PRIO, "capi_gain_module: PARAM_ID_MODULE_ENABLE, %lu", me_ptr->gain_config.enable);
         }
         else
         {
            AR_MSG(DBG_ERROR_PRIO,
                   "capi_gain_module: PARAM_ID_MODULE_ENABLE, Bad param size %lu",
      ...
      case PARAM_ID_GAIN_MODULE_GAIN:
      {
         if (params_ptr->actual_data_len >= sizeof(param_id_module_gain_cfg_t))
         {
            ...
            capi_result |= capi_gain_module_raise_process_event(me_ptr, enable);

            AR_MSG(DBG_HIGH_PRIO, "capi_gain_module: PARAM_ID_GAIN %d", me_ptr->gain_config.gain_q13);
         }
         else
         {
            AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Set, Bad param size %lu", params_ptr->actual_data_len);
      ...
      default:
      {
         AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Set, unsupported param ID 0x%x", (int)param_id);
...
/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_get_param
                                             ...
                                             capi_buf_t *            params_ptr)
{
   ...
   if (NULL == _pif || NULL == params_ptr)
   {
      AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Get param received bad pointer, 0x%p, 0x%p", _pif, params_ptr);
      return CAPI_EBADPARAM;
   }
   ...
   switch (param_id)
   {
      case PARAM_ID_MODULE_ENABLE:
      {
         ...
         {
            AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Get Enable Param, Bad payload size %d", params_ptr->max_data_len);
            capi_result = CAPI_ENEEDMORE;
         }
         break;
      }
      case PARAM_ID_GAIN_MODULE_GAIN:
      {
         ...
         {
            AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Get, Bad param size %lu", params_ptr->max_data_len);
            capi_result = CAPI_ENEEDMORE;
         }
         break;
      }
      default:
      {
         AR_MSG(DBG_ERROR_PRIO, "capi_gain_module: Get, unsupported param ID 0x%x", param_id);
...
/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_set_properties
 * Function to set Sets a list of property values.
 * -------------------------------------------------------------------------*/
static capi_err_t capi_gain_module_set_properties(capi_t *_pif, capi_proplist_t *props_ptr)
{
   capi_gain_module_t *me_ptr = (capi_gain_module_t *)_pif;
   return capi_gain_module_process_set_properties(me_ptr, props_ptr);
}

/* -------------------------------------------------------------------------
 * Function name: capi_gain_module_get_properties
 * Function to get a list of property values.
 * -------------------------------------------------------------------------*/
static capi_err_t capi_gain_module_get_properties(capi_t *_pif, capi_proplist_t *props_ptr)
{
   capi_gain_module_t *me_ptr = (capi_gain_module_t *)_pif;
   return capi_gain_module_process_get_properties(me_ptr, props_ptr);
}
